...
public class PDFViewer extends JFrame
    ...
    public final static String TITLE = "تحويل ملف PDF  إلى صور";
    /** The current PDFFile */
    PDFFile curFile;
    ...
    /// FILE MENU
    Action openAction = new AbstractAction("فتح...", getIcon("iconspx48/folder_open.png")) {

        public void actionPerformed(ActionEvent evt) {
            doOpen();
        }
    };
    Action pageSetupAction = new AbstractAction("إعدادات الصفحة...", getIcon("iconspx48/utilities_folder.png")) {

        public void actionPerformed(ActionEvent evt) {
            doPageSetup();
        }
    };
    Action printAction = new AbstractAction("طباعة...", getIcon("iconspx48/printers_faxes.png")) {

        public void actionPerformed(ActionEvent evt) {
            doPrint();
        }
    };
    Action closeAction = new AbstractAction("إغلاق", getIcon("iconspx48/public_folder.png")) {

        public void actionPerformed(ActionEvent evt) {
            doClose();
        }
    };
    Action quitAction = new AbstractAction("خروج", getIcon("iconspx48/public_folder_badged.png")) {
...
//        }
//    }
//    ZoomAction zoomInAction = new ZoomAction("تكبير الحجم",
//            getIcon("gfx/zoomin.gif"),
//            2.0);
//    ZoomAction zoomOutAction = new ZoomAction("تصغير الحجم",
...
//        }
//    };
//    Action fitInWindowAction = new AbstractAction("تكبير حسب حجم الشاشة",
//            getIcon("iconspx48/window_fullscreen.png")) {
//
//        public void actionPerformed(ActionEvent evt) {
//            doFitInWindow();
//        }
//    };
    ...
    class ThumbAction extends AbstractAction
        ...
        public ThumbAction() {
            super("إخفاء الصور المصغرة",getIcon("iconspx48/Navigation_Pane.png"));
        }
        ...
        public void propertyChange(PropertyChangeEvent evt) {
            ...
            if (v <= 1) {
                isOpen = false;
                putValue(ACTION_COMMAND_KEY, "إظهار الصور المصغرة");
                putValue(NAME, "إظهار الصور المصغرة");
            } else {
                isOpen = true;
                putValue(ACTION_COMMAND_KEY, "إخفاء الصور المصغرة");
                putValue(NAME, "إخفاء الصور المصغرة");
            }
        }
    ...
    Action fullScreenAction = new AbstractAction("عرض حسب حجم الشاشة",
            getIcon("iconspx48/window_fullscreen.png")) {
    ...
    Action extractToImagesJPG = new AbstractAction("تحويل لصيغة JPG",
            getIcon("iconspx48/jpeg_image.png")) {

        public void actionPerformed(ActionEvent evt) {
            doExtractToImages("jpg");//(evt.getModifiers() & evt.SHIFT_MASK) != 0);
        }
    };

    Action extractToImagesBMP = new AbstractAction("تحويل لصيغة BMP",
            getIcon("iconspx48/bmp_image.png")) {

        public void actionPerformed(ActionEvent evt) {
            doExtractToImages("bmp");//(evt.getModifiers() & evt.SHIFT_MASK) != 0);
        }
    };

    Action extractToImagesPNG = new AbstractAction("تحويل لصيغة PNG",
            getIcon("iconspx48/png_image.png")) {

        public void actionPerformed(ActionEvent evt) {
            doExtractToImages("png");//(evt.getModifiers() & evt.SHIFT_MASK) != 0);
        }
    };

    Action extractToImagesGIF = new AbstractAction("تحويل لصيغة GIF",
            getIcon("iconspx48/gif_image.png")) {

        public void actionPerformed(ActionEvent evt) {
            doExtractToImages("gif");//(evt.getModifiers() & evt.SHIFT_MASK) != 0);
        }
    };

    Action nextAction = new AbstractAction("التالي", getIcon("iconspx48/forward.png")) {

        public void actionPerformed(ActionEvent evt) {
            doNext();
        }
    };
    Action firstAction = new AbstractAction("الأول", getIcon("iconspx48/go_first.png")) {

        public void actionPerformed(ActionEvent evt) {
            doFirst();
        }
    };
    Action lastAction = new AbstractAction("الأخير", getIcon("iconspx48/finish.png")) {

        public void actionPerformed(ActionEvent evt) {
            doLast();
        }
    };
    Action prevAction = new AbstractAction("السابق", getIcon("iconspx48/back.png")) {
    ...
    Action aboutAction = new AbstractAction("عن البرنامج",
            getIcon("iconspx48/info.png")) {

        public void actionPerformed(ActionEvent evt) {
            showMessageDialog(null, "برنامج لعرض و تحويل ملفات ال PDF  الى صور دون الحاجة لبرامج اخرى.\n افتح ملف ثم عالجه باختيار قائمة التصدير");
        }
    };
    ...
    public PDFViewer(boolean useThumbs) {
        ...
        URL url = getClass().getResource("gfx/crystal_java.png");
    ...
    protected void init() {
        ...
        pageField.setMaximumSize(new Dimension(70, 70));
        ...
        //jtb.setText("");
        //bg.add(jtb);
        //toolbar.add(jtb); dont add the zoom
...
//        toolbar.add(jtb);
        ...
        file.add(quitAction);
        mb.add(file);
        JMenu view = new JMenu("عرض");
        JMenu process = new JMenu("معالجة");
        process.add(extractToImagesJPG);
        process.add(extractToImagesBMP);
        process.add(extractToImagesGIF);
        process.add(extractToImagesPNG);
        process.setEnabled(true);
        JMenu about = new JMenu("عن");
        about.add(aboutAction);
        JMenu zoom = new JMenu("الحجم");
        //zoom.add(zoomInAction);
        //zoom.add(zoomOutAction);
        //zoom.add(fitInWindowAction);
        zoom.setEnabled(true);
        //view.add(zoom); dont add the zoom
...
//        System.out.println("Going to page " + pagenum);
    ...
    public void setEnabling() {
        ...
        //zoomToolAction.setEnabled(pageshown); dont enable the zoom
    ...
    private void openPDFByteBuffer(ByteBuffer buf, String path, String name) {
        ...
        try {
            ...
            openError(path + " الملف الذي تم اختياره ليس من نوع PDF ." +
                      "\n: " + ioe.getMessage ());
            return;
        }
    ...
    public void openError(String message) {
        showMessageDialog(split, message, "حدث خطأ ",
    ...
    class PrintThread extends Thread {
        ...
        public void run() {
            try {
                ...
                showMessageDialog(PDFViewer.this,
                        "خطأ في عملية الطباعة: " + pe.getMessage(),
                        "تم إيقاف عملية الطباعة",
                        JOptionPane.ERROR_MESSAGE);
            }
            ptPages.hide();
        }
    ...
    public void doExtractToImages(String extension){
        if(curFile != null) {
            String path = null;
            for (int i = 0; i < curFile.getNumPages(); i++) {
                try {
                    path = "D:\\" + extension + "_" + docName + "_" + i + "." + extension;
                    createImage(curFile.getPage(i), "D:\\" + extension + "_" + docName + "_" + i + "." + extension);
                } catch (IOException e) {
                    e.printStackTrace();
                }

                showMessageDialog(null, "تتم تصدير الملف رقم " +i+ " للمسار "+ path);
            }
        }
    }
